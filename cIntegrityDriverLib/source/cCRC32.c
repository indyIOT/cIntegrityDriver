/*********************************************************************
 * @file cCRC32.c
 * @brief Source file for the CRC32 implementation. Mirrors the structure of
 *        cCRC16.c: a configurable bit-by-bit calculator for the general case,
 *        and a fast table-driven calculator for the standard reflected
 *        CRC-32/ISO-HDLC polynomial (the "CRC32" used by zlib/Ethernet/PNG)
 *        for embedded-optimized builds.
 ...
 *============================================================================*/
static const uint8_t moduleName[] = "cCRC32";
#define MODULE_ID 56526
...
/*
 * crc32_ieee_tab - table for the reflected CRC-32/ISO-HDLC polynomial
 * (0xEDB88320, the bit-reversal of 0x04C11DB7). Same table used by zlib,
 * Ethernet FCS, and PNG. check("123456789") = 0xCBF43926.
 * Table cost: 1024 bytes flash. One lookup + shift + xor per byte.
 */
...
/**
 * @brief Reflects the bits of a 32-bit value.
 * @param v The 32-bit value to reflect.
 * @return The reflected 32-bit value.
 */
static uint32_t reflect32( uint32_t v )
{
    uint32_t r = 0;
                                ...
                                size_t length, uint32_t * const crc32Value )
{
    ...
    else
    {
        ...
        while ( length-- )
        {
            ...
            for ( int i = 0; i < 8; i++ )
            {
                crc = ( crc & 0x80000000u ) ? ( uint32_t )( ( crc << 1 ) ^ config->poly )
                                        : ( uint32_t )( crc << 1 );
            }
        }
        ...
        crc ^= config->xorOut;
#else
        crc = CRC32TableIEEE( buffer, length, CRC_32_SEED ) ^ CRC_32_XOROUT;
#endif
    *crc32Value = crc;
    }
    return ( retValue );
...
/**
 * @brief Calculates the CRC32 of a given buffer using the CRC-32/ISO-HDLC polynomial.
 * @param buffer Pointer to the buffer to calculate the CRC32 of.
 * @param length The length of the buffer in bytes.
 * @param initialCrc The initial CRC value to use for the calculation.
 * @return The calculated CRC32 value.
 */
uint32_t CRC32TableIEEE( void const * const buffer,
                         size_t const length,
